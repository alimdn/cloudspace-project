...
name: file-doctor
description: >
  Diagnose, analyze, and fix errors in computer files including code files, configuration files,
  data files, and log files. Use this skill whenever the user asks to: check a file for errors,
  fix a broken file, debug a code file, repair corrupted JSON/YAML/XML, fix syntax errors,
  analyze error logs, troubleshoot file issues, detect encoding problems, validate file formats,
  or any task involving finding and resolving problems in files. Also triggers on phrases like
  "this file has errors", "fix this file", "why isn't this working", "debug this code",
  "repair this config", "check for errors", "something is wrong with my file",
  "error in file", "fix syntax", "validate file", "check log file", "troubleshoot",
  "diagnose issue", or similar. Covers text files, code files (.py, .js, .ts, .html, .css,
  .java, .cpp, .go, .rb, .php, .sh), config files (.json, .yaml, .yml, .xml, .ini, .toml,
  .env, .conf), data files (.csv, .tsv, .log), and markup files (.md, .rst, .txt).
  This skill reads files, detects errors, explains issues clearly, and applies fixes with
  user confirmation. It handles encoding issues, syntax errors, structural corruption,
  formatting problems, and provides detailed diagnostic reports.
compatibility:
  tools:
    - Read
    - Write
